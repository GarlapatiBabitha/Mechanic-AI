...
const Sidebar = ({ user, onNewChat, onChatSelect }) => {
  ...
  const handleRename = async (chatId) => {
    if (newTitle.trim()) {
      ...
      try {
        // Await the rename API call
        const data = await ChatAPI.renameTitle(userId, chatId, newTitle);
  
        // Safeguard in case the response format isn't as expected
        if (data?.success) {
          console.log("Rename successful:", data);
        } else {
          console.error("Rename failed:", data?.error || "Unexpected error");
          alert(data?.error || "Failed to rename chat. Please try again.");
          // Rollback the UI change if the rename failed
          setChats((prevChats) =>
            prevChats.map((chat) =>
              chat.sessionId === chatId
                ? { ...chat, title: chat.title } // Restore old title
                : chat
            )
          );
        }
        ...
        // Rollback the UI change if an error occurs
        setChats((prevChats) =>
          prevChats.map((chat) =>
            chat.sessionId === chatId
              ? { ...chat, title: chat.title } // Restore old title
    ...
    <div className="sidebar">
      ...
      {extended && (
        <div className="recent">
          ...
          {!hasMore && <p className="no-more-chats">No more chats available</p>}
        </div>
      )}
    </div>
  );
